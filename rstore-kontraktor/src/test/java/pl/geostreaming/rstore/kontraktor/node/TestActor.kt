...
import pl.geostreaming.rstore.kontraktor.node.impl.RsNodeActorImpl
...
//            .withSlotSize(50)
    ...
    fun makeAct(id:Int):Triple<RsNodeActor, RsNodeActor, ActorServer>{
        val act = Actors.AsActor(RsNodeActorImpl::class.java)
    ...
    //                .serType(enc);
            ...
            return Triple(act, actRem,pp);
        }
        else {

            val pp = WebSocketPublisher(act, "localhost", "/xx", port)
                    .serType(enc)
                    .publish().await();
            val con = WebSocketConnectable(RsNodeActor::class.java, "http://localhost:${port}/xx").serType(enc);
            val actRem = con.connect<RsNodeActor> { _, _ -> println("disconnect") }.await()
...
//        }
        ...
        acts.forEach { (_,_,x) -> x.close().await() }
